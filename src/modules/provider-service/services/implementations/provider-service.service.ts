...
export class ProviderServiceService implements IProviderServiceService {
  ...
  async createService(
    providerId: string,
    userType: UserType,
    createServiceDto: CreateProviderServiceDto,
  ...
  async updateService(
    ...
    userType: UserType,
    serviceId: string,
    updateServiceDto: UpdateProviderServiceDto,
    file: Express.Multer.File,
  ): Promise<IResponse<IProviderServiceUI>> {
    const updateData = Object.fromEntries(
      Object.entries(updateServiceDto).filter(([_, value]) => value !== undefined && value !== null),
    ...
    if (file) {
      ...
      const publicId = this._uploadUtility.getPublicId(userType, providerId, UploadsType.SERVICE, categoryId!);
  ...
  async toggleStatus(serviceId: string): Promise<IResponse> {
    ...
    if (service.isActive === false && (service.categoryId?.isActive === false || service.professionId?.isActive === false)) {
      throw new BadRequestException({
