...
import {
  ICoupon,
  ICouponAppliedResponse,
  ICouponFilter,
  ICouponTableData,
  ICouponWithPagination,
} from '@core/entities/interfaces/coupon.entity.interface';
...
export class CouponService implements ICouponService {
  ...
  async getAllCoupons(couponFilterDto: CouponFilterDto): Promise<IResponse<ICouponWithPagination>> {
    ...
    return {
      success: true,
      message: coupons.length > 0 ? 'All coupons fetched successfully.' : 'No Coupons found.',
      data: {
        coupons,
        pagination: {
          total: totalCoupons,
  ...
  async getAvailableCoupons(): Promise<IResponse<ICoupon[]>> {
    ...
    const coupons = (couponDocs ?? [])
      .filter((coupon) => {
        ...
        if (coupon.usageType === UsageTypeEnum.Expiry && coupon.validTo && !this._isCouponExpired(coupon.validTo)) return true;
        return false;
      })
  ...
  async applyCoupon(applyCouponPayload: ApplyCouponPayloadDto): Promise<IResponse<ICouponAppliedResponse>> {
    ...
    if (coupon.usageType === UsageTypeEnum.Expiry && coupon.validTo && this._isCouponExpired(coupon.validTo)) {
      throw new BadRequestException({
        code: ErrorCodes.BAD_REQUEST,
