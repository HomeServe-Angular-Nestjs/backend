...
export class LandingService implements ILandingService {
  ...
  async getLandingData(): Promise<IResponse<ILandingData>> {
    ...
    const providers = featuredProviders.map((provider) => ({
      ...provider,
      avatar: provider.avatar ? this._uploadsUtility.getSignedImageUrl(provider.avatar) : '',
    }));

    const reviewTestimonials = testimonials.map((testimonial) => ({
      ...testimonial,
      customerAvatar: testimonial.customerAvatar ? this._uploadsUtility.getSignedImageUrl(testimonial.customerAvatar, 140) : '',
    }));
