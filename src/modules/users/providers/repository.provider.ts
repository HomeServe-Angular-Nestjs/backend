...
import type { BookingDocument } from '@/core/schema/bookings.schema';
...
export const adminRepositoryProviders: Provider[] = [
  {
    ...
    useFactory: (customerModel: Model<CustomerDocument>) => new CustomerRepository(customerModel),
    inject: [getModelToken(CUSTOMER_MODEL_NAME)],
  },
  {
    provide: TRANSACTION_REPOSITORY_NAME,
    useFactory: (bookingModel: Model<BookingDocument>) => new TransactionRepository(bookingModel),
    inject: [getModelToken(BOOKINGS_MODEL_NAME)],
  },
  {
    provide: PROVIDER_REPOSITORY_INTERFACE_NAME,
    useFactory: (providerModel: Model<ProviderDocument>) => new ProviderRepository(providerModel),
    inject: [getModelToken(PROVIDER_MODEL_NAME)],
  },
  {
    provide: BOOKING_REPOSITORY_NAME,
    useFactory: (bookingModel: Model<BookingDocument>) => new BookingRepository(bookingModel),
    inject: [getModelToken(BOOKINGS_MODEL_NAME)],
  },
  {
  ...
  },
  {
    ...
    useFactory: (weeklyAvailabilityModel: Model<WeeklyAvailabilityDocument>) => new WeeklyAvailabilityRepository(weeklyAvailabilityModel),
    inject: [getModelToken(WEEKLY_AVAILABILITY_MODEL_NAME)],
  },
  {
    provide: DATE_OVERRIDES_REPOSITORY_INTERFACE_NAME,
    useFactory: (dateOverrideModel: Model<DateOverrideDocument>) => new DateOverridesRepository(dateOverrideModel),
    inject: [getModelToken(DATE_OVERRIDE_MODEL_NAME)],
  },
  {
    provide: WALLET_LEDGER_REPOSITORY_NAME,
    useFactory: (walletLedgerModel: Model<WalletLedgerDocument>) => new WalletLedgerRepository(walletLedgerModel),
  ...
  },
  {
    ...
    useFactory: (settingsModel: Model<AdminSettingsDocument>) => new AdminSettingsRepository(settingsModel, new LoggerFactory()),
