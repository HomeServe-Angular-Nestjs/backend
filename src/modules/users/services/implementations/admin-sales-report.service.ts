...
export class AdminSalesReportService implements IAdminSalesReportService {
    ...
    private _dateRangeLabel(from?: string, to?: string): string {
        const fmt = (d?: string) => d ? new Date(d).toLocaleDateString('en-IN', { year: 'numeric', month: 'short', day: 'numeric' }) : 'Present';
    ...
    private async _buildWorkbook(report: ISalesReportBundle): Promise<Buffer> {
        ...
        cancellationSheet.addRow({ cancelledOrders: report.cancellation.cancelledOrders, cancellationRate: report.cancellation.cancellationRate });
