...
export class RazorPaymentService implements IRazorPaymentService {
  ...
  private async _settleBookingPayment(
    orderData: BookingOrderData,
    user: ITxUserDetails & { id: string },
  ...
  private async _settleSubscriptionPayment(
    userDetails: ITxUserDetails & { id: string },
  ...
  private async _applyBookingPaymentWalletMovements(
    userId: string,
    role: UserType,
    orderData: BookingOrderData,
    ...
    const userWalletUpdated = await this._walletRepository.updateUserAmount(userId, role, -transaction.amount);
  ...
  private async _applySubscriptionPaymentWalletMovements(
    userId: string,
    role: UserType,
    orderData: SubscriptionOrderData,
    transaction: ITransaction,
  ...
  async handleBookingPayment(
    ...
    try {
      ...
      await this._applyBookingPaymentWalletMovements(userId, role, orderData, transaction, verifyData);
  ...
  async handleSubscriptionPayment(
    ...
    try {
      const [subscription, user] = await Promise.all([this._getSubscription(orderData.subscriptionId), this._getUser(userId, role)]);
      ...
      await this._applySubscriptionPaymentWalletMovements(userId, role, orderData, transaction, verifyData);
