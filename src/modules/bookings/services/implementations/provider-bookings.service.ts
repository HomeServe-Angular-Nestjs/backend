...
import {
  PAYMENT_LOCKING_UTILITY_NAME,
  PRICING_UTILITY_NAME,
  TIME_UTILITY_NAME,
  UPLOAD_UTILITY_NAME,
...
import {
  ...
  IBookedSlot,
  IBooking,
  IBookingDetailProvider,
  IBookingInvoice,
  IBookingOverviewChanges,
  IBookingOverviewData,
  IPriceBreakupData,
  IProviderBookingListService,
  IResponseProviderBookingLists,
  IReviewDetails,
  IReviewFilter,
  IReviewWithPagination,
...
export class ProviderBookingService implements IProviderBookingService {
  ...
  async fetchBookingsList(providerId: string, page: number = 1, filters: FilterFields): Promise<IResponseProviderBookingLists> {
    ...
    const { data, total } = await this._bookingRepository.fetchProviderBookingsWithPagination(providerId, filters, { page, limit });
  ...
  async fetchOverviewData(providerId: string, scope: 'month' | 'total' = 'month'): Promise<IBookingOverviewData> {
    ...
    const bookingsThisMonth = bookings.filter(
      (b) => b.createdAt !== undefined && b.createdAt >= thisMonthRange.start && b.createdAt <= thisMonthRange.end,
    );

    const bookingsLastMonth = bookings.filter(
      (b) => b.createdAt !== undefined && b.createdAt >= lastMonthRange.start && b.createdAt <= lastMonthRange.end,
    );
  ...
  async markBookingCancelledByProvider(providerId: string, bookingId: string, reason?: string): Promise<IResponse<IBookingDetailProvider>> {
    const bookingDoc = await this._bookingRepository.findPaidBookings(bookingId);
    if (!bookingDoc) {
      this.logger.error(`Booking ${bookingId} is paid or it is not in the db.`);
    ...
    if (!customer) {
      throw new NotFoundException({
        code: ErrorCodes.RESOURCE_NOT_FOUND,
        message: `Customer with ID ${bookingDoc.customerId.toString()} not found.`,
      });
    }
    ...
    const alreadyRefunded =
      bookingDoc.transactionHistory.filter(
        (t) => t.transactionType === TransactionType.BOOKING_REFUND && t.status === TransactionStatus.SUCCESS,
      ).length > 0;
  ...
  async downloadBookingInvoice(bookingId: string, userType: ClientUserType): Promise<Buffer> {
    ...
    const invoiceData: IBookingInvoice = {
      ...
      transactionId:
        booking.transactionHistory.find(
          (t) => t.transactionType === TransactionType.BOOKING_PAYMENT && t.status === TransactionStatus.SUCCESS,
        )?.id ?? null,
      paymentStatus: booking.paymentStatus,
      ...
      paymentDetails:
        transaction && transaction.gateWayDetails
          ? {
  ...
  async completeBooking(providerId: string, bookingId: string): Promise<IResponse<IBookingDetailProvider>> {
    ...
    try {
      ...
      if (!isLocked)
        throw new BadRequestException({
          code: PaymentErrorCodes.PAYMENT_IN_PROGRESS_ERROR,
          message: PaymentErrorMessages.PAYMENT_IN_PROGRESS,
        });
      ...
      if (bookingDoc.providerId.toString() !== providerId) {
        throw new BadRequestException({
          ...
          message: BookingErrorMessages.UNAUTHORIZED_ACCESS,
        });
      }
      ...
      if (
        ...
        !booking.transactionHistory.some((t) => t.transactionType === TransactionType.BOOKING_PAYMENT)
      )
        throw new BadRequestException({
          code: BookingErrorCodes.ACTION_FORBIDDEN_UNPAID,
          message: BookingErrorMessages.ACTION_FORBIDDEN_UNPAID,
        });

      if (booking.bookingStatus === BookingStatus.COMPLETED) {
        throw new BadRequestException({
          code: BookingErrorCodes.BOOKING_ALREADY_COMPLETED,
          message: BookingErrorMessages.BOOKING_ALREADY_COMPLETED,
        });
      }
      ...
      let [adminWallet, providerWallet] = await Promise.all([this._getAdminWallet(), this._getUserWallet(booking.providerId)]);

      if (!adminWallet || !providerWallet)
      ...
      const commissionAmount = Math.round(baseAmount * (providerCommissionInPercentage / 100));
      const providerAmount = baseAmount - commissionAmount;
      const gstAmount = totalAmount - baseAmount;

      const bookingReleaseTransaction = await this._transactionRepository.createNewTransaction(
        bookingId,
        this._transactionMapper.toDocument({
      ...
      if (adminWallet.balance < providerAmount) {
        throw new BadRequestException({
          code: WalletErrorCodes.INSUFFICIENT_BALANCE_ERROR,
          message: WalletErrorMessages.ADMIN_WALLET_INSUFFICIENT_BALANCE,
        });
      }

      const adminWalletUpdate = await this._walletRepository.updateAdminAmount(-providerAmount);
      if (!adminWalletUpdate) {
        this.logger.error('Admin wallet update failed for the release of booking ' + bookingId);
        throw new InternalServerErrorException({
          code: WalletErrorCodes.AMOUNT_DEBIT_ERROR,
          message: WalletErrorMessages.AMOUNT_DEBIT_FAILED,
        });
      }

      const providerWalletUpdate = await this._walletRepository.updateUserAmount(providerWallet.userId, 'provider', providerAmount);
      if (!providerWalletUpdate) {
        await this._walletRepository.updateAdminAmount(providerAmount);
        this.logger.error('Provider wallet update failed for the release of booking ' + bookingId);
        throw new InternalServerErrorException({
          code: WalletErrorCodes.AMOUNT_CREDIT_ERROR,
          message: WalletErrorMessages.AMOUNT_CREDIT_FAILED,
        });
      }
      ...
      if (!updatedBooking) {
        await this._walletRepository.updateAdminAmount(providerAmount);
        await this._walletRepository.updateUserAmount(providerWallet.userId, 'provider', -providerAmount);
        this.logger.error('Booking update failed for the release of booking ' + bookingId);
        throw new InternalServerErrorException({
          code: BookingErrorCodes.BOOKING_RELEASE_FAILED,
          message: BookingErrorMessages.BOOKING_RELEASE_FAILED,
        });
      }
      ...
      await this._walletLedgerRepository.create(
        this._walletLedgerMapper.toDocument({
          ...
          balanceAfter: adminWallet.balance - providerAmount,
          journalId,
          bookingId,
      ...
      const customerCommission = transaction.metadata?.breakup?.customerCommission || 0;
      ...
      providerWallet = await this._getUserWallet(booking.providerId);
  ...
  async rescheduleBooking(providerId: string, bookingId: string, slotData: SelectedSlotDto): Promise<IResponse<IBookingDetailProvider>> {
    const bookingDoc = await this._bookingRepository.findById(bookingId);
    if (!bookingDoc)
      throw new NotFoundException({
        code: ErrorCodes.NOT_FOUND,
    ...
    const hasConflict = this._hasSlotConflict(bookingDate, bookingOfSameProviderInSameDate, slotData.from, slotData.to);
    ...
    const isReserved = await this._reservationRepository.isReserved(booking.providerId, slotData.from, slotData.to, bookingDate);
  ...
  private _hasSlotConflict(date: Date, existingBookings: IBooking[], newFrom: string, newTo: string): boolean {
    ...
    const newEnd = this._timeUtility.apply24hTime(date, newTo);
    ...
    return existingBookings.some((booking) => {
      ...
      const existingEnd = this._timeUtility.apply24hTime(existingDate, booking.slot.to);
  ...
  private async _handleWalletUpdateOnBookingCancellation(
    providerId: string,
    customerId: string,
    bookingId: string,
    ...
    const adminWallet = await this._getAdminWallet();
    const [customerWallet, providerWallet] = await Promise.all([this._getUserWallet(customerId), this._getUserWallet(providerId)]);
  ...
  private async _getBookedServices(servicesIds: string[]): Promise<IBookedService[]> {
    return await Promise.all(
      servicesIds.map(async (s) => {
        ...
        if (!service) throw new InternalServerErrorException(ErrorMessage.INTERNAL_SERVER_ERROR);
  ...
  private _computeRefund(
    totalPaid: number,
    adminSettings: IAdminSettings,
  ): { refundAmount: number; customerFine: number; providerFine: number } {
