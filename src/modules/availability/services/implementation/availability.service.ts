...
export class AvailabilityService implements IAvailabilityService {
  ...
  private _validateDayAvailability(day: IDayAvailability, dayName?: string): void {
    ...
    const intervals = timeRanges.map((slot) => {
      ...
      if (start >= end) {
        throw new BadRequestException({
          ...
          message: dayName ? `${dayName}: startTime must be before endTime` : 'startTime must be before endTime',
        });
      }
    ...
    for (let i = 1; i < intervals.length; i++) {
      ...
      if (currStart < prevEnd) {
        throw new BadRequestException({
          code: ErrorCodes.BAD_REQUEST,
          message: dayName ? `${dayName}: time ranges must not overlap` : 'time ranges must not overlap',
        });
      }
    }
  }
  ...
  async updateWeeklyAvailability(
    providerId: string,
    updateWeeklyAvailabilityDto: UpdateWeeklyAvailabilityDto,
  ): Promise<IResponse<IWeeklyAvailability>> {
  ...
  async fetchDateOverrides(providerId: string): Promise<IResponse<IDateOverride[]>> {
    ...
    const overrides = dateOverridesDoc.map((override) => this._availabilityMapper.toDateOverrideEntity(override));
  ...
  async deleteDateOverride(providerId: string, date: string): Promise<IResponse> {
    ...
    if (Number.isNaN(dateSent.getTime()))
      throw new BadRequestException({
        code: ErrorCodes.BAD_REQUEST,
        message: 'Invalid date',
      });
