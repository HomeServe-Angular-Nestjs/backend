...
export const availabilityRepositoryProviders: Provider[] = [
  {
    ...
    useFactory: (availabilityModel: Model<WeeklyAvailabilityDocument>) => new WeeklyAvailabilityRepository(availabilityModel),
    inject: [getModelToken(WEEKLY_AVAILABILITY_MODEL_NAME)],
  },
  {
    provide: DATE_OVERRIDES_REPOSITORY_INTERFACE_NAME,
    useFactory: (overrideModel: Model<DateOverrideDocument>) => new DateOverridesRepository(overrideModel),
