...
export class CartService implements ICartService {
  ...
  async addItem(customerId: string, providerId: string, providerServiceId: string): Promise<IResponse<ICartPopulated>> {
    ...
    if (!service || service.isActive === false || service.categoryId?.isActive === false || service.professionId?.isActive === false) {
    ...
    const updatedCartDoc = await this._cartRepository.findAndPopulateByCustomerId(customerId);
    if (!updatedCartDoc)
      throw new NotFoundException({
        code: ErrorCodes.NOT_FOUND,
        message: 'Cart not found',
