...
export class ProviderController {
  ...
  async fetchProviders(@User() user: IPayload, @Query() filterDto: FilterDto) {
    ...
    return await this._providerServices.getProviders({
      ...filter,
      page: Number(page),
      limit: Number(limit),
    });
  ...
  async fetchOneProvider(
    @User() user: IPayload,
    @Query() query: { providerId: string | null },
  ): Promise<IResponse<ICustomerProviderDetails>> {
    let userId = user.sub;
  ...
  async bulkUpdateProvider(
    @User() user: IPayload,
    @Body('providerData') providerDetailsDto: string,
  ...
  async fetchAvailabilitySlots(
    @User() user: IPayload,
    @Param('id', new isValidIdPipe()) providerId: string,
    @Query() { date }: DateDto,
  ): Promise<IResponse<ISlotUI[]>> {
  ...
  async fetchRescheduleSlots(
    @Query() { providerId, selectedDate, totalDurationInMinutes }: RescheduleSlotsDto,
  ): Promise<IResponse<ISlotUI[]>> {
