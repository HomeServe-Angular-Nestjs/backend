...
export class ProviderServices implements IProviderServices {
    ...
    async updateProvider(user: IPayload, updateData: Partial<IProvider>, file: Express.Multer.File): Promise<Provider> {
        const avatarUrl = await this.cloudinaryService.uploadImage(file);

        if (!avatarUrl) {
            throw new NotFoundException('Avatar not updated to cloudinary successfully');
        }
