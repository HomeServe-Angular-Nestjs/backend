...
import {
  ...
  IProviderPerformanceOverview,
  IProviderRevenueOverview,
  IResponseTimeChartData,
  IReviewChartData,
  IPerformanceAnalyticsBundle,
  IRevenueAnalyticsBundle,
...
export class ProviderAnalyticsService implements IProviderAnalyticsService {
    ...
    return {
      ...
      data: {
        ...
        reviews: recentReviews.map((r) => ({
          name: r.customerId && typeof r.customerId === 'object' && 'username' in r.customerId ? (r.customerId as any).username : 'Unknown',
          desc: r.review?.desc || '',
  ...
  async getComparisonStats(providerId: string): Promise<IResponse<IComparisonChartData[]>> {
    ...
    const fullYearData: IComparisonChartData[] = Array.from({ length: 12 }, (_, i) => {
  ...
  async getRevenueTrendOverTime(providerId: string, view: RevenueChartView): Promise<IResponse<IRevenueTrendData>> {
    ...
    if (view === 'monthly') labels = monthLabels;
      ...
      const labelsSet = new Set(allYears);
  ...
  async getPerformanceBundle(providerId: string): Promise<IResponse<IPerformanceAnalyticsBundle>> {
    const [summary, bookings, quality, comparison] = await Promise.all([
      ...
      Promise.all([this.getPerformanceBookingOverview(providerId), this.getPerformanceTrends(providerId)]),
      Promise.all([
      ...
      Promise.all([this.getComparisonOverviewData(providerId), this.getComparisonStats(providerId)]),
    ]);
  ...
  async getAreaBundle(providerId: string): Promise<IResponse<IAreaAnalyticsBundle>> {
    const [summary, demand, revenue, peak] = await Promise.all([
      ...
      Promise.all([this.getServiceDemandData(providerId), this.getServiceDemandByLocation(providerId)]),
      Promise.all([this.getTopAreasRevenue(providerId), this.getUnderperformingAreas(providerId)]),
