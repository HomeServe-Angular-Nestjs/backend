...
export class CartRepository extends BaseRepository<CartDocument> implements ICartRepository {
  ...
  async clearCartByCustomerId(customerId: string): Promise<boolean> {
    const result = await this._cartModel.updateOne({ customerId: this._toObjectId(customerId) }, { $set: { items: [] } });
  ...
  async isTheSameProviderInCart(customerId: string, providerId: string): Promise<boolean> {
    ...
    return result[0].providers.length === 1 && result[0].providers[0].toString() === providerId;
  }
}
