...
import {
  IAdminLedgerRecord,
  ICustomerTransactionData,
  IProviderTransactionData,
  IProviderTransactionOverview,
  ITransactionStats,
...
export class WalletLedgerRepository extends BaseRepository<WalletLedgerDocument> implements IWalletLedgerRepository {
  ...
  private _buildTransactionFilterQuery(
    filters: IWalletTransactionFilter,
    userId?: string,
  ): { match: FilterQuery<WalletLedgerDocument>; sort: SortQuery<WalletLedgerDocument> } {
  ...
  private _buildAdminBaseMatch(filters: IWalletTransactionFilter): FilterQuery<WalletLedgerDocument> {
    const match: FilterQuery<WalletLedgerDocument> = {
      $or: [{ userRole: 'admin' }, { type: TransactionType.SUBSCRIPTION_PAYMENT, direction: PaymentDirection.CREDIT }],
    };
    ...
    return [
      {
        $match: {
          $or: [
            ...
            regexMatch({
              $ifNull: [{ $arrayElemAt: ['$customer.email', 0] }, { $arrayElemAt: ['$provider.email', 0] }, ''],
            }),
          ],
        },
      },
  ...
  async countFiltered(filters: IWalletTransactionFilter): Promise<number> {
    const pipeline: PipelineStage[] = [...this._buildAdminStages(filters), { $count: 'total' }];
  ...
  async getFilteredCustomerLedgersByUserIdWithPagination(
    userId: string,
    filters: IWalletTransactionFilter,
    options: { page: number; limit: number },
  ): Promise<ICustomerTransactionData[]> {
  ...
  async getFilteredProviderLedgersByUserIdWithPagination(
    userId: string,
    filters: IWalletTransactionFilter,
    options: { page: number; limit: number },
  ): Promise<IProviderTransactionData[]> {
    const page = options?.page && options.page > 0 ? options.page : 1;
    const limit = options?.limit && options.limit > 0 ? options.limit : 10;
    const skip = (page - 1) * limit;
    ...
    const pipeline: PipelineStage[] = [
      { $match: { userId: this._toObjectId(providerId) } },
      {
        $group: {
          _id: null,
          totalCredit: {
            $sum: { $cond: [{ $eq: ['$direction', PaymentDirection.CREDIT] }, '$amount', 0] },
          },
          totalDebit: {
            $sum: { $cond: [{ $eq: ['$direction', PaymentDirection.DEBIT] }, '$amount', 0] },
          },
        },
      },
      {
  ...
  async getAdminTransactionLists(
    filters: IWalletTransactionFilter,
    options: { page: number; limit: number },
  ): Promise<IAdminLedgerRecord[]> {
    const page = options?.page && options.page > 0 ? options.page : 1;
    const limit = options?.limit && options.limit > 0 ? options.limit : 10;
    const skip = (page - 1) * limit;
    ...
    const pipeline: PipelineStage[] = [
      ...
      { $limit: limit },
      {
        $project: {
          ...
          gatewayPaymentId: 1,
          counterpartyEmail: {
            $ifNull: [{ $arrayElemAt: ['$customer.email', 0] }, { $arrayElemAt: ['$provider.email', 0] }, null],
          },
        },
      },
    ];
        ...
        $group: {
          ...
          providerPayouts: {
            $sum: {
              $cond: [
                {
                  $and: [{ $eq: ['$userRole', 'provider'] }, { $eq: ['$direction', PaymentDirection.CREDIT] }],
                },
                '$amount',
                0,
              ],
