...
export class ProviderServiceRepository extends BaseRepository<ProviderServiceDocument> implements IProviderServiceRepository {
  ...
  async createAndPopulate(doc: ProviderServiceDocument): Promise<ProviderServicePopulatedDocument> {
    ...
    const populated = await this._providerServiceModel.findById(created._id).populate('professionId').populate('categoryId').exec();

    return populated as unknown as ProviderServicePopulatedDocument;
  }

  async updateAndPopulateByServiceId(
    serviceId: string,
    update: Partial<IProviderService>,
  ): Promise<ProviderServicePopulatedDocument | null> {
    return await this._providerServiceModel
      .findByIdAndUpdate(serviceId, { $set: update }, { new: true })
      .populate('professionId')
      .populate('categoryId')
      .lean<ProviderServicePopulatedDocument | null>();
  }

  async findAllAndPopulateByProviderId(
    providerId: string,
      ...
      if (activeOnly) {
        const [facet] = await this._providerServiceModel.aggregate<{
          ...
          { $sort: sort },
          {
            $facet: {
              total: [{ $count: 'count' }],
              docs: [{ $skip: skip }, { $limit: options.limit }],
            },
          },
        ]);
  ...
  async updateStatusByServiceId(serviceId: string): Promise<boolean> {
    const updated = await this._providerServiceModel.updateOne({ _id: this._toObjectId(serviceId), isDeleted: false }, [
