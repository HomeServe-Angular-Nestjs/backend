...
export class CouponRepository extends BaseRepository<CouponDocument> implements ICouponRepository {
  constructor(private readonly _couponModel: Model<CouponDocument>) {
    super(_couponModel);
  }

  async fetchCouponsWithFilterAndPagination(
    couponFilter: ICouponFilter,
    ...
    return await this._couponModel.find(matchQuery).sort({ createdAt: -1 }).skip(skip).limit(limit).exec();
  }

  async editCoupon(couponId: string, updateData: Partial<ICoupon>): Promise<CouponDocument | null> {
    return await this._couponModel.findOneAndUpdate({ _id: couponId }, { $set: updateData }, { new: true });
  }
  ...
  async deleteCouponById(couponId: string): Promise<boolean> {
    const result = await this._couponModel.updateOne({ _id: couponId }, { $set: { isDeleted: true, isActive: false } });
    return result.modifiedCount > 0;
  }

  async toggleStatusById(couponId: string): Promise<boolean> {
    const result = await this._couponModel.updateOne({ _id: couponId }, [
