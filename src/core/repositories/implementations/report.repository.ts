...
export class ReportRepository extends BaseRepository<ReportDocument> implements IReportRepository {
    ...
    const pipeline: PipelineStage[] = [
      ...
      ...(filter.search
        ? [
            ...
            { $sort: { searchPriority: 1 as const, createdAt: -1 as -1 } },
          ]
  ...
  async updateInvestigationNotes(reportId: string, investigationNotes: string): Promise<ReportDocument | null> {
    return await this._reportModel.findByIdAndUpdate(reportId, { $set: { investigationNotes } }, { new: true });
  }
  ...
  async getReportOverviewDetails(): Promise<IReportOverViewMatrix> {
    const statusOverviewPipeline: PipelineStage[] = [
      {
      ...
      },
      {
        $replaceRoot: {
          newRoot: {
            $mergeObjects: [{ total: '$total' }, { $arrayToObject: '$counts' }],
          },
        },
      },
    ];

    const flaggedPipeline: PipelineStage[] = [
