...
export interface ISchedulesRepository extends IBaseRepository<SchedulesDocument> {
    isExists(filter: FilterQuery<SchedulesDocument>): Promise<boolean>;
    count(filter?: FilterQuery<SchedulesDocument>): Promise<number>;
    updateOne(filter: FilterQuery<SchedulesDocument>, update: UpdateQuery<SchedulesDocument>): Promise<{ matchedCount: number; modifiedCount: number }>;
}